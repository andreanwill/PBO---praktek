...
package UASsoalB.dao;

import UASsoalB.model.Taxi;
import java.sql.*;
import java.util.*;
...
public class TaxiDb 
{
    ...
    public void update(Taxi taxi) throws Exception {
        ...
        String query = "update taxi set kodepenumpang=?, nama=?, jenispenumpang=?, "
                ...
                + "biayaperkilo=?, totalbayar=? where kodetransaksi=?";
        PreparedStatement ps = db.getKoneksi().prepareStatement(query);
        ...
        ps.setString(8, taxi.getTotalBayar());
        ps.setString(9, taxi.getKodeTransaksi());
        ps.executeUpdate();
    }
    ...
    public Taxi getTaxi(String kodeTransaksi) throws Exception{
        ...
        if(rs.next()){
            ...
            taxi.setTotalBayar(rs.getString(9));
        }
        return taxi;
    }
